...
print_feedback("Loading seeds...") do
  ...
  sql_files = Dir[File.join(Rails.root, 'db', 'seeds', '*.sql')].sort
  ...
  csv_files = Dir[File.join(Rails.root, 'db', 'seeds', '*.csv')].sort
  ...
  ruby_scripts = Dir[File.join(Rails.root, 'db', 'seeds', '*.rb')].sort
